...
* Menambahkan user student ke group docker (opsional tapi direkomendasikan)
    Langkah ini penting agar setiap perintah docker tidak perlu diawali dengan sudo.
    
    ```bash
    sudo usermod -aG docker $USER
    ```

    Logout dan login kembali agar perubahan group membership berlaku. Perintah berikut juga dapat digunakan agar perubahan langsung berlaku tanpa perlu melakukan login ulang:
    ```bash
    newgrp docker
    ```
...
* Ubah menjadi
    ...
    Perubahan IP ini diperlukan agar DVWA dapat diakses melalui host lain yang ada di jaringan Host-Only VirtualBox.
...
* **Login Dashboard**
    ...
    Jika lupa password, reset akun administrator dengan perintah:

    ```bash
    docker exec safeline-mgt resetadmin
    ```

    Perintah tersebut akan menampilkan username dan password administrator. Gunakan username dan password tersebut untuk login di dashboard SafeLine.

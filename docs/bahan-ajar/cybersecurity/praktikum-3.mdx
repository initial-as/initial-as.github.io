...
```bash
sudo usermod -aG adminlab operator
```
...
```bash
sudo chown operator praktikum3/rahasia.txt
```

Ubah owner dan group:

```bash
sudo chown operator:adminlab praktikum3/rahasia.txt
```
...
* hanya akun **operator** yang dapat login melalui SSH.
